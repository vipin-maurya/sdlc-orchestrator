...
`BLOCKED_ON_QUOTA`, `BLOCKED_ON_NETWORK`, `ESCALATED`, `CANCELLED`,
`TIMED_OUT`, `FAILED`.
...
| SCOPING | agent retries exhausted | ESCALATED |
| AWAITING_SCOPE_APPROVAL | `sdlc approve` | PLANNING (open questions waived) |
| AWAITING_SCOPE_APPROVAL | `sdlc reject`, rounds < `limits.max_scope_rounds` | SCOPING (answers attached) |
| AWAITING_SCOPE_APPROVAL | `sdlc reject`, rounds >= `limits.max_scope_rounds` | ESCALATED (resumes into SCOPING) |
| AWAITING_SCOPE_APPROVAL | `sdlc reject --cancel` | CANCELLED |
...
| any agent state | backend unreachable (transport failure) | BLOCKED_ON_NETWORK (auto-resumes to the same state after `resume_after`) |
...
- BLOCKED_ON_NETWORK does **not** consume retry/fix budgets either. A backend
  that cannot be reached is not the agent getting the work wrong, which is what
  `limits.max_agent_retries` exists to bound. Classification is the CLI's own
  `terminal_reason` where it reports one, otherwise
  `backends.*.transport_error_patterns` matched against the CLI's error message
  — never against the agent transcript, which routinely discusses network
  errors as ordinary subject matter. The job's `hold_reason` carries the
  backend's own sentence so the cause is visible without opening the log.
