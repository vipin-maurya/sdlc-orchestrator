...
- **A7** — `Span` gains `Children []Span`, so emphasis can wrap inline code. §2.4 had spans flat, which forced the four asides that quote a command or a filename to carry literal backticks inside `SpanEmphasis.Text`. Markdown reads that correctly; HTML does not, and the browser showed grave accents where a reader expects monospace. The markdown bytes are unchanged — the goldens pass either way, which is why this needed a test on the HTML side rather than relying on them. Nesting is not general: nothing in the package builds a span more than two deep, and the parts are given structurally at the call site rather than recovered by scanning for backticks, which would be the implicit contract the block model exists to remove. W3.
