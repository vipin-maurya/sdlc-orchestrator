...
func TitleAndBody(title, content string) (title2, body string) {
	...
	// the two agree; this way the line itself survives whichever path ran. It
	// is trimmed on both paths: a body that carried the file's trailing
	// newline only when the file had one line would be a difference nobody
	// meant between two issue files.
