...
func TestConfigPageRedactsCredentials(t *testing.T) {
	...
	// A credential can be short. The value rule used to skip anything under
	// eight characters outright, so this one printed in the clear.
	const shortToken = "hunter2"
	t.Setenv("SDLC_TEST_API_TOKEN", envToken)
	t.Setenv("SDLC_TEST_SHORT_PASSWORD", shortToken)
	...
	tgt.Ship.ResumeCommand = []string{"autoship", "resume", envToken, shortToken}
	e.cfg.Targets["demo"] = tgt

	body := getOK(t, e, "/config")
	for _, secret := range []string{flagToken, envToken, shortToken} {
